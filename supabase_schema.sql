...
CREATE POLICY "All users can read categories" ON categories
...
CREATE POLICY "Cashiers can update product stock" ON products
    FOR UPDATE USING (
...
CREATE POLICY "Cashiers can create sale_items" ON sale_items
    FOR INSERT WITH CHECK (
        auth.uid() IN (SELECT id FROM users WHERE role = 'cashier')
    );

CREATE POLICY "Users can read their own sale_items" ON sale_items
    FOR SELECT USING (
        auth.uid() IN (
...
CREATE POLICY "Admins can do anything with receipts" ON receipts
    USING (auth.uid() IN (SELECT id FROM users WHERE role = 'admin'))
    WITH CHECK (auth.uid() IN (SELECT id FROM users WHERE role = 'admin'));

CREATE POLICY "Cashiers can create receipts" ON receipts
    FOR INSERT WITH CHECK (
        auth.uid() IN (SELECT id FROM users WHERE role = 'cashier')
    );

CREATE POLICY "Users can read their own receipts" ON receipts
    FOR SELECT USING (
        auth.uid() IN (
            SELECT user_id FROM sales WHERE id = receipts.sale_id
        )
    );
